...
import { getMatingPets } from '@/api/petApi';
...
function parseAgeYears(age: string | null): number | null {
  if (!age) return null;
  const years = age.match(/(\d+)\s*year/i);
  const months = age.match(/(\d+)\s*month/i);
  let total = 0;
  let found = false;
  if (years) { total += parseInt(years[1], 10); found = true; }
  if (months) { total += parseInt(months[1], 10) / 12; found = true; }
  return found ? total : null;
}

function compatibilityScore(a: PetResponse, b: PetResponse): number {
  let score = 0;
  if (a.species === b.species) score += 40;
  ...
  if (a.location && b.location && a.location === b.location) score += 10;
  return score;
}
...
export default function MatingPage() {
  ...
  const recommended = useMemo(() => {
    ...
    const scored: { pet: PetResponse; score: number }[] = [];
    for (let i = 0; i < pets.length; i++) {
      let bestScore = 0;
      for (let j = 0; j < pets.length; j++) {
        if (i === j) continue;
        if (pets[i].gender === pets[j].gender) continue;
        const s = compatibilityScore(pets[i], pets[j]);
        if (s > bestScore) bestScore = s;
      }
      scored.push({ pet: pets[i], score: bestScore });
    }
    return scored
        ...
        <section className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          >
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('mating.searchPh')}
                className="w-full rounded-2xl border-2 border-gray-200 bg-white py-3 pe-10 ps-11 font-body text-gray-800 placeholder:text-gray-400 transition-all focus:border-ocean-400 focus:outline-none focus:ring-4 focus:ring-ocean-100"
              />
              {search && (
                <button
                  onClick={() => setSearch('')}
            ...
            <div className="mt-4 hidden items-center gap-3 md:flex">
              ...
              <select
                ...
                className="rounded-full border-2 border-gray-200 bg-white px-4 py-2 font-body text-sm text-gray-700 transition-all focus:border-ocean-400 focus:outline-none"
              >
                <option value="">{t('mating.allTypes')}</option>
                {SPECIES_LIST.map((s) => (
              ...
              <select
                ...
                className="rounded-full border-2 border-gray-200 bg-white px-4 py-2 font-body text-sm text-gray-700 transition-all focus:border-ocean-400 focus:outline-none"
              >
                <option value="">{t('mating.allGenders')}</option>
              ...
              <select
                ...
                className="rounded-full border-2 border-gray-200 bg-white px-4 py-2 font-body text-sm text-gray-700 transition-all focus:border-ocean-400 focus:outline-none"
              >
                <option value="">{t('mating.allLocations')}</option>
                {locations.map((loc) => (
          ...
          {!loading && !error && (
            <p className="mt-4 mb-2 font-body text-sm font-medium text-gray-500">
              {resultCount === 1
                ? t('mating.result', { count: resultCount })
                : t('mating.results', { count: resultCount })}
            </p>
          )}
        </section>

        {/* Recommended Matches */}
        {!loading && !error && recommended.length > 0 && !hasActiveFilters && (
          <section className="mx-auto max-w-6xl px-4 pt-8 sm:px-6">
            <div className="mb-5 flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-accent-500" />
              <div>
                <h2 className="font-display text-xl font-bold text-gray-800">
                  {t('mating.recommended')}
                </h2>
                <p className="font-body text-sm text-gray-500">
                  {t('mating.recommendedDesc')}
                </p>
              </div>
            </div>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {recommended.map((pet, i) => (
                <MatingCard key={pet.id} pet={pet} index={i} highlighted />
              ))}
            </div>
          </section>
        )}
        ...
        <section className="mx-auto max-w-6xl px-4 pb-16 pt-8 sm:px-6">
          {loading && (
            <div className="flex flex-col items-center justify-center py-24">
              <LoadingSpinner size={40} label={t('mating.loading')} />
            </div>
          )}
          ...
          {!loading && !error && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-ocean-50">
                <PawPrint className="h-10 w-10 text-ocean-300" />
              </div>
              <h2 className="font-display text-xl font-semibold text-gray-700">
                {t('mating.noResults')}
              </h2>
              <p className="mt-2 max-w-sm font-body text-gray-500">
                {t('mating.noResultsDesc')}
              </p>
              {hasActiveFilters && (
                <button
                  onClick={clearFilters}
                  className="mt-5 flex items-center gap-1.5 rounded-full bg-ocean-50 px-5 py-2.5 font-display text-sm font-semibold text-ocean-600 transition-all hover:bg-ocean-100"
                >
                  <X className="h-4 w-4" />
                  {t('mating.clearFilters')}
                </button>
          ...
          {!loading && !error && filtered.length > 0 && (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((pet, i) => (
                <MatingCard key={pet.id} pet={pet} index={i} />
              ))}
            </div>
          )}
        </section>
...
function MatingCard({
  ...
  return (
    <motion.div
      ...
      className={`group flex flex-col overflow-hidden rounded-3xl bg-white shadow-lg ring-1 transition-all hover:-translate-y-1 hover:shadow-xl ${
        highlighted ? 'ring-accent-200' : 'ring-gray-100'
      }`}
    >
      {/* Image */}
      <Link to={`/pet/${pet.id}`} className="relative block overflow-hidden bg-gray-50">
        ...
        {/* Mating badge */}
        <div className="absolute top-3 start-3 flex items-center gap-1.5 rounded-full bg-ocean-500/90 px-3 py-1.5 font-body text-xs font-semibold text-white shadow-md backdrop-blur">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mb-3">
          ...
          {pet.breed ? (
            ...
            <p className="font-body text-sm text-gray-400">
              {t('mating.breed')}: {t('mating.unknown')}
            </p>
          )}
        </div>
        ...
        <div className="mt-auto flex items-center gap-2">
          <Link
            to={`/pet/${pet.id}`}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-2xl bg-ocean-50 px-4 py-2.5 font-display text-sm font-semibold text-ocean-600 transition-all hover:bg-ocean-100 active:scale-95"
          >
            {t('mating.viewProfile')}
          ...
          {hasWhatsapp && (
            <a
              ...
              aria-label={t('mating.contactOwner')}
            >
